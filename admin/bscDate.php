        ...
        $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=1 "
                . " GROUP BY u.idUsuario ");
        $row = mysqli_fetch_array($qr);
        @$entradas = $row['total'];

        $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=2  "
                . " GROUP BY u.idUsuario  ");
        $row = mysqli_fetch_array($qr);
        @$saidas = $row['total'];
        ...
        <div class="row mb-2">
            ...
            $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                    . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=1 && MONTH(dataMovimentacao)='$mes' && YEAR(dataMovimentacao)='$ano' "
                    . " GROUP BY u.idUsuario ");
            $row = mysqli_fetch_array($qr);
            @$entradas = $row['total'];

            $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                    . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=2 && MONTH(dataMovimentacao)='$mes' && YEAR(dataMovimentacao)='$ano'"
                    . " GROUP BY u.idUsuario ");
            $row = mysqli_fetch_array($qr);
            @$saidas = $row['total'];
$datahoje = date('Y-m-d');
...
<!--MOVIMENTO DA SEMANA
            ...
            $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                    . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=1 && WEEK(dataMovimentacao)='$semana' && MONTH(dataMovimentacao)='$mes' && YEAR(dataMovimentacao)='$ano' "
                    . " GROUP BY u.idUsuario ");
            $row = mysqli_fetch_array($qr);
            @$entradas = $row['total'];

            $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                    . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=2 && WEEK(dataMovimentacao)='$semana' && MONTH(dataMovimentacao)='$mes' && YEAR(dataMovimentacao)='$ano' "
                    . " GROUP BY u.idUsuario ");
            $row = mysqli_fetch_array($qr);
            @$saidas = $row['total'];
...
<!--MOVIMENTO DO DIA-->
            ...
            $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                    . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=1 && DAY(dataMovimentacao)='$dia' && MONTH(dataMovimentacao)='$mes' && YEAR(dataMovimentacao)='$ano' "
                    . " GROUP BY u.idUsuario ");
            $row = mysqli_fetch_array($qr);
            @$entradas = $row['total'];

            $qr = mysqli_query($conn, "SELECT SUM(m.valorMovimentacao) as total FROM movimentacao m"
                    . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE m.tipoMovimentacao=2 && DAY(dataMovimentacao)='$dia' && MONTH(dataMovimentacao)='$mes' && YEAR(dataMovimentacao)='$ano' "
                    . " GROUP BY u.idUsuario ");
            $row = mysqli_fetch_array($qr);
            @$saidas = $row['total'];
            ...
            <div class="col-md-6">
                <div class="card flex-md-row mb-4 shadow-sm h-md-250">
                    <div class="card-body d-flex flex-column align-items-start">
                        ...
                        <div class="mb-1 text-muted">Resultado das movimentacões do dia atual (<?php echo strftime("%A, %d de %B de %Y", strtotime($datahoje)); ?> )</div>
        ...
        <main role="main" class="container">
            ...
            <div id="formAddCat" style="display: none;" >
                ...
                <table  class="table">
                    <form method="post" action="index.php?p=procCCat">
                        <input type="hidden" name="acao" value="2" />  
                         <input type="hidden" name="idLogado" value="<?php echo $_SESSION['id']?>" /> 
                ...
                <?php
                $qr = mysqli_query($conn, "SELECT * FROM usuario u INNER JOIN categoriacaixa c ON c.categoriacaixaid_usuarioid  = " . $_SESSION['id'] . ""
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                while ($row = mysqli_fetch_array($qr)) {
                                    ?>  
                                    <td><?php echo $row['nomeCategoriaCaixa'] ?></td>
...
//if(isset($_GET['myModalC'])){
            $qrr = mysqli_query($conn, "SELECT * FROM usuario u INNER JOIN categoriacaixa c ON c.categoriacaixaid_usuarioid = " . $_SESSION['id'] . ""
                    . " GROUP BY c.categoriacaixa_nome "
                    . " ORDER BY c.categoriacaixa_nome ASC"
                    . "");
            ?>
            ...
            while ($row = mysqli_fetch_array($qrr)) {
                ?>  
                <div id="myModalC<?php echo $row['idCategoriaCaixa'] ?>" class="modal fade" role="dialog">
                            ...
                            <div class="modal-body">
                                <form role="form" method="POST" action="index.php?p=editCat">
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <div class="form-line">
                                                <input type="hidden" name="idR" class="form-control" required placeholder="Digite uma senha minimo 5 digitos" value="<?php echo $row['categoriacaixa_id '] ?>" >
                                            ...
                                            <div class="form-line">
                                                <input type="text" class="form-control" required placeholder="Digite uma senha minimo 5 digitos" value="<?php echo $row['categoriacaixa_nome'] ?>" name="categoria">
...
<input type="hidden" value="fc" name="p" />
                        ...
                        <input type="hidden" value="<?php echo $ano ?>" name="ano" />             
                             ...
                             <?php }else  { ?>     
                            <input type="hidden" value="bdate" name="p" />       
                                

                            <input type="hidden" value="<?php echo $_GET['datainicio'] ?>" name="datainicio" />
                            <input type="hidden" value="<?php echo $_GET['datafim'] ?>" name="datafim" />
...
<!-- -->
                        ...
                        if (isset($_GET['mes'])) {
                            ...
                            $sqlano = $_GET['ano'];
                            $qr = mysqli_query($conn, "SELECT * FROM usuario u INNER JOIN categoriacaixa c ON c.categoriacaixaid_usuarioid = " . $_SESSION['id'] . ""
                                    . " INNER JOIN movimentacao m ON categoriaMovimentacao_idCategoria = idCategoriaCaixa WHERE MONTH(m.dataMovimentacao) = $sqlmes && YEAR(m.dataMovimentacao) = $sqlano  "
                                     . " GROUP BY c.categoriacaixa_nome "
                        . " ORDER BY c.categoriacaixa_nome ASC"
                                    . "");
                        }
                        elseif (isset($_GET['datainicio'])) {
                            ...
                            $qr = mysqli_query($conn, "SELECT * FROM usuario u INNER JOIN categoriacaixa c ON c.categoriacaixaid_usuarioid  = " . $_SESSION['id'] . ""
                                    . " INNER JOIN movimentacao m ON categoriaMovimentacao_idCategoria = idCategoriaCaixa WHERE m.dataMovimentacao BETWEEN ('$datainicio') AND ('$datafim')  "
                                   . " GROUP BY c.categoriacaixa_nome "
                        . " ORDER BY c.categoriacaixa_nome ASC"
                                    . "");
                        }
                            ...
                            $qr = mysqli_query($conn, "SELECT * FROM movimentacao m INNER JOIN categoriacaixa c ON m.categoriaMovimentacao_idCategoria = c.categoriacaixa_id "
                                    . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE MONTH(m.dataMovimentacao) = $mes && YEAR(m.dataMovimentacao) = $ano "
                                    . " GROUP BY c.idCategoriaCaixa "
                                    . "   ORDER BY m.dataMovimentacao DESC");
                        }
...
//                      }
                         if  (isset($_GET['datainicio']) && isset($_GET['datafim']) && isset($_GET['idCat']) && (!empty($_GET['idCat']))) {
                            ...
                            $kok = $_GET['idCat']; 
                                    $resultado_sql = "SELECT * FROM movimentacao m INNER JOIN categoriacaixa c ON m.categoriaMovimentacao_idCategoria = $kok "
                                            . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario =  $id WHERE c.categoriacaixa_id = $kok AND m.dataMovimentacao BETWEEN ('$datainicio') AND ('$datafim')  "
                            ...
                            elseif  (isset($_GET['mes'])  && isset($_GET['idCat']) && (!empty($_GET['idCat']))) {
                                    $sqlano = $_GET['ano'];
                                    $sqlmes = $_GET['mes'];
                                    $id = $_SESSION['id'];
                                    $kok = $_GET['idCat'];
                                            ...
                                            <?php if ($resultado['tipoMovimentacao'] == 1 || $resultado['tipoMovimentacao'] == 2) { ?>          
                                                <td> <?php echo $data->format('d/m/Y') ?></td>
                                                 ...
                                                 ?>
                                                      ...
                                                      <td><a href="index.php?p=bdate&datainicio=<?php echo $datainicio ?>&datafim=<?php echo $datafim ?>&idCat=<?php echo $resultado['categoriaMovimentacao_idCategoria'] ?>"><?php echo $resultado['nomeCategoriaCaixa'] ?></a></td>                  
...
<?php } }?>
                ...
                <?php
                if ((isset($_GET['mes'])) || isset($_GET['datainicio']) || isset($_GET['idCat']) || isset($resultado['idCategoria'])) {
                    $datainicio = $_GET['datainicio'];
                    $datafim = $_GET['datafim'];
                    //$mesul = $_GET['mes'];
                    $resultado_sql = "SELECT * FROM movimentacao m INNER JOIN categoriacaixa c ON  c.usuarioCategoria_idUsuario =  " . $_SESSION['id'] . ""
                            . " INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . " WHERE  m.dataMovimentacao BETWEEN ('$datainicio') AND ('$datafim')  "
                            . " GROUP BY m.idMovimentacao "
                            . "   ORDER BY m.dataMovimentacao DESC";
                }
                @$data = new DateTime($resultados['dataMovimentacao']);
                                ...
                                <div class="modal-body">
                                    <form role="form" method="POST" action="index.php?p=proceditarM">
                                        ...
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                <div class="form-line">
                                                    <input type="hidden" name="idR" class="form-control" required placeholder="Digite uma senha minimo 5 digitos" value="<?php echo $resultados['idMovimentacao'] ?>" >
                                                ...
                                                <?php
                                                $qr = mysqli_query($conn, "SELECT * FROM usuario u INNER JOIN categoriacaixa c ON c.usuarioCategoria_idUsuario = " . $_SESSION['id'] . ""
                                                        . " GROUP BY c.nomeCategoriaCaixa "
                                                        . " ORDER BY c.nomeCategoriaCaixa ASC"
                                                        . "");
                                                ?>
                                                ...
                                                <div class="">
                                                    <select name="categoria">
                                                        ...
                                                        while ($row = mysqli_fetch_array($qr)) {
                                                            ...
                                                            ?>
                                                            <option <?php if (isset($idCatnovo) && $idCatnovo == $row['idCategoriaCaixa']) echo "selected=selected" ?> value="<?php echo $row['idCategoriaCaixa'] ?>"><?php echo $row['nomeCategoriaCaixa'] ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            ...
                                            Descrição:
                                            <div class="form-line">
                                                <input type="text" class="form-control" required placeholder="Digite no minimo 5 digitos" value="<?php echo $resultados['nomeMovimentacao'] ?>" name="nome">
                ...
                <aside class="col-md-4 blog-sidebar">
                    <div class="p-3">
                        ...
                        <ol class="list-unstyled mb-0">
                            <?php
                            $qr = mysqli_query($conn, "SELECT MONTH(m.dataMovimentacao) as 'mes', YEAR(m.dataMovimentacao) as 'ano'  FROM movimentacao m INNER JOIN usuario u ON m.usuarioMovimentacao_idUsuario = " . $_SESSION['id'] . ""
                                    . " GROUP BY  mes, ano "
